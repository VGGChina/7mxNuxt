...
var utilHelper = {
  ...
  score(media) {
    ...
    try {
      let likeNum = media.like_num ? parseInt(media.like_num) : 0
      let lookNum = media.look_num ? parseInt(media.look_num) : 0
      let commentNum = media.comment_num ? parseInt(media.comment_num) : 0
      let recommend = media.recommend ? parseInt(media.recommend) : 0
      let product = 0
      ...
      let x = 0.9 + lookNum / 10 + likeNum * 2 + commentNum * 2 + product * 50 + recommend * 50

      let score = 70 + Math.log(600 * (1 - 1 / (x / 500 + 1))) / Math.log(10) * 10
